<template>
  <view class="page-wrap">
    ...
    <view class="mode-tabs">
      <view
        :class="['tab-item', mode==='in' ? 'tab-in-active' : '']"
        @tap="setMode('in')"
      >📦 入库</view>
      <view
        :class="['tab-item', mode==='out' ? 'tab-out-active' : '']"
        @tap="setMode('out')"
      >🚚 出库</view>
    </view>
    ...
    <view class="card scan-card">
      <view class="scan-row">
        <input
          ...
          @confirm="handleInput"
        />
        <button class="btn btn-green scan-cam-btn" @tap="openIOSScanner">
    ...
    <view v-if="currentProduct" :class="['card product-card', mode==='in' ? 'card-in' : 'card-out']">
      <view class="product-header">
        ...
        <view
          class="stock-chip"
          :style="{ background: currentProduct.stock <= currentProduct.warn_stock ? '#ffebee' : '#e8f5e9' }"
        >
      ...
      <button
        :class="['btn btn-block', mode==='in' ? 'btn-green' : 'btn-amber']"
    ...
    <view v-if="notFound" class="card not-found-card">
      ...
      <button class="btn btn-green btn-block" style="margin-top:24rpx" @tap="goAdd">
        ➕ 立即录入该商品
      </button>
    </view>
...
// ── iOS 原生扫码 ──────────────────────────────
function openIOSScanner() {
  // #ifdef APP-PLUS
  const barcode = plus.barcode

  // iOS 使用 plus.barcode.scan，自动调用 AVFoundation
  barcode.scan(
    (type, result) => {
      vibrate('short')
    ...
    // 扫码界面样式（iOS 自定义）
    {
      frameColor: '#1a5c2a',   // 扫码框颜色
      scanbarColor: '#1a5c2a', // 扫描线颜色
      background: '#000',
    }
  )
  // #endif
  ...
  // #ifdef H5
  uni.showToast({ title: '请在真机上使用摄像头扫码', icon: 'none' })
  // #endif
}
...
function adjustQty(delta) {
  qty.value = Math.max(1, (qty.value || 1) + delta)
}
...
async function confirmBatch() {
  ...
  try {
    ...
    const payload = {
      type:  mode.value,
      items: scanStore.batchItems.map(i => ({
        barcode:    i.barcode,
        qty:        i.qty,
...
function goAdd() {
  uni.navigateTo({ url: `/pages/inventory/add?barcode=${notFoundBarcode.value}` })
}
</script>
...
/* iOS 状态栏占位（NavigationBar 为 custom 时使用） */
.ios-status-bar {
  height: 0;
  /* #ifdef APP-PLUS */
  /* 由系统状态栏自动处理，此处留空 */
...
.scan-cam-btn { width: 96rpx; flex-shrink: 0; }
...
.qty-btn  {
  ...
  font-size: 36rpx; font-weight: 700; color: var(--text);
}
.qty-input {
  width: 120rpx; height: 64rpx; text-align: center;
  font-size: 36rpx; font-weight: 700; color: var(--text);
  background: var(--gray-card); border-radius: 12rpx;
}
...
.batch-row {
  display: flex; align-items: center;
  padding: 20rpx 0; border-bottom: 1rpx solid var(--border);
}
.batch-info  { flex: 1; }
